$(document).ready(function(){
	...
	$("body").on("click", "#add_main_task input.btn-primary", function(){
		...
		// if($("#main_tasks li.active_main_task").length == 1)
		// 	$("#main_tasks li.active_main_task").after(main_task_template(add_main_data));
		// else
		...
		// setTimeout(function(){
		// 	if($("#sprint_cycle_sort li[data-task-id='"+id_generator+"']").closest("#sprint_cycle_sort").length == 1 && $("#sprint_cycle_sort li[data-task-id='"+id_generator+"'] .main_task_status_select option:selected").val() == 2){
		// 		$("#main_tasks li[data-task-id="+id_generator+"]").find(".main_task_status_select").selectpicker("destroy");
		// 		$("#main_tasks li[data-task-id="+id_generator+"]").find(".main_task_detail_select").selectpicker("destroy");

		// 		var li_pending = $("#main_tasks li[data-task-id='"+id_generator+"']").clone();
		// 		$("#main_tasks li[data-task-id='"+id_generator+"']").remove();
		// 		$("#product_backlog_sort").append(li_pending);

		// 		$(li_pending).find(".main_task_detail_select").selectpicker();
		// 		$(li_pending).find(".main_task_status_select").selectpicker().selectpicker('val', 2);

		// 		update_main_task_index(".main_sort");
		// 	}

		// 	countdown_to_saved = 5;
		// 	save_changes();
		// }, 4000)
	});
	...
	$("body").on("keyup", ".estimated_points input, .task_title input", function(){
		...
		for (var i = 0; i < tasks.length; i++) {
			if(tasks[i].task_id == input_value.closest("li").attr("data-task-id")){
				if(input_value.closest(".estimated_points").length == 1)
					tasks[i].estimated_points = input_value.val();
				else
		...
		if(task != -1) {
			...
			for (var i = 0; i < projects.length; i++) {
				if(_.difference(project_ids).indexOf(projects[i].value) == -1)
					append_project += '<option value="'+projects[i].value+'" data-color-tag="'+projects[i].color+'">'+projects[i].name+'</option>';
			}
		...
		for (var i = 0; i < tasks.length; i++) {
			if(tasks[i].task_id == project_tag.closest("#right_content").attr("data-main-task-id") && project_tag.hasClass("project_tag_see_more")){
				for (var x = 0; x < tasks[i].sub_project_tag.length; x++) {
					if(x != 0){
						append_tags += '<li data-tag-id="'+tasks[i].sub_project_tag[x].project_id+'" data-tag-color="'+tasks[i].sub_project_tag[x].project_color+'" class=""><span>'+tasks[i].sub_project_tag[x].project_name+'</span><span hidden="true" class="remove_project_tag glyphicon glyphicon-remove pull-right"></span></li>';
					}
				}
			}
				...
				for (var x = 0; x < tasks[i].sub_project_tag.length; x++) {
					append_tags += '<li data-tag-id="'+tasks[i].sub_project_tag[x].project_id+'" data-tag-color="'+tasks[i].sub_project_tag[x].project_color+'" class=""><span>'+tasks[i].sub_project_tag[x].project_name+'</span></li>';
				}
			}
		}
...
function show_main_tasks(){
	...
	for (var i = 0; i < tasks.length; i++) {
		...
		if(json_data.task_detail == 1  || json_data.task_detail == 3){
			append_current_task += main_task_template(json_data);
		}
		else if(json_data.task_detail == 2 || json_data.task_detail == 3)
...
var sub_task_template = _.template('<li data-sub-task-id="<%= sub_task_id %>">\
								<form action="">\
									<div class="task_container">\
										<div class="task_index">\
											<span class="index_count"><%= index %></span>\
						...
						<ul class="sortable ui-sortable subtask_sort" id="remaining_task_sort">\
							<% for(var i = 0; i < sub_tasks.length; i++) {  %>\
								<% if(sub_tasks[i].task_detail != 3){  %>\
									<li data-sub-task-id="<%= sub_tasks[i].sub_task_id %>">\
										<form action="">\
											<div class="task_container">\
												<div class="task_index">\
